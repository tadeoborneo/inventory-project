const AddProductForm = ({ newProduct, setNewProduct, onSubmit, isEditing, isLoading }) => {
    const isInvalid = !newProduct.name || newProduct.stock < 0 || newProduct.price <= 0 || newProduct.lead_time_days <= 0 || newProduct.cost <= 0
        || isNaN(newProduct.stock) || isNaN(newProduct.price) || isNaN(newProduct.lead_time_days) || isNaN(newProduct.cost);
    ...
    return (
        <form onSubmit={onSubmit} className="bg-white p-6 rounded-xl shadow-md mb-8 grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            ...
            <button
                ...
                className={`font-bold py-2 px-4 rounded-lg transition-colors ${isEditing
